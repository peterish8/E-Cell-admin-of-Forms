...
export function InteractiveFormRenderer({ form }: InteractiveFormRendererProps) {
  ...
  const checkEmailDuplicate = async (email: string) => {
    ...
    if (existing) {
      setError("This email has already been used to submit this form")
      return false
    }
    return true
  }

  const handleAnswer = async (questionId: string, value: any, questionType: string) => {
    if (questionType === 'multiple') {
      ...
      // Check for email duplicates in real-time
  ...
  const validateAnswer = async () => {
    ...
    if (currentQuestion.question_type === 'email') {
      ...
      const isUnique = await checkEmailDuplicate(answer)
      if (isUnique === false) {
        return false
      }
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Final email duplicate check before submission
      if (userEmail) {
        const isUnique = await checkEmailDuplicate(userEmail)
        if (isUnique === false) {
          throw new Error("This email has already been used to submit this form")
        }
      }
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
          ...
          <div className="space-y-4 mb-6">
            ...
            {(['text', 'email'].includes(currentQuestion.question_type)) && (
              <Input
                type={currentQuestion.question_type === 'email' ? 'email' : currentQuestion.question_id.toLowerCase().includes('phone') ? 'tel' : 'text'}
                name={
